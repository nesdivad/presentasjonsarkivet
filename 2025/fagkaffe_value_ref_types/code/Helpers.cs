...
{
    public static Table RenderStack(Stack<string> stack, int? capacity = null)
    {
        var items = stack.ToArray();
        var table = new Table().LeftAligned().Border(TableBorder.Heavy);

        table.AddColumn(
            new TableColumn($"[orangered1]Stack (cap: {stack.Capacity}[/])").Centered()
        );
